...
-0b101_010                # 2進数（負）
```
また、ASCII文字やエスケープシーケンスに対応する整数値を取得するためには、その前に`?`を付けて記述できます。制御文字やメタ文字の組み合わせも、`?\C-x`、`?\M-x`、`?\M-\C-x`を使って生成できます。制御バージョンの値は`value & 0x9f`、メタバージョンの値は`value | 0x80`で生成されます。最後に、`?\C-?`はASCIIの削除（0177）を生成します。
```rb
?a                        # 文字コード
?\n                       # 改行コード (0x0a)
...
?\C-?                     # 削除文字
```
小数点や指数を含む数値リテラルは、Floatオブジェクトに変換され、これはネイティブアーキテクチャの倍精度浮動小数点型に対応します。小数点の後には必ず数字が必要です。例えば、`1.e3`は`Fixnum`クラスのメソッド`e3`を呼び出そうとしてしまうためエラーになります。
